...
public class Editor_helper
{
    ...
    [MenuItem("EditorHelper/LoadDataSO")]
    static void LoadAndCallMethod()
    {
        // Load tất cả các ScriptableObject trong thư mục Resources
        Sprite_FXSkill_SO[] scriptableObjects = Resources.LoadAll<Sprite_FXSkill_SO>("TextLoad/FX_skill/");
        Debug.Log( "scriptableObjects :  "+ scriptableObjects.Length);
        if (scriptableObjects.Length == 0)
        {
            Debug.LogWarning("No ScriptableObjects found in Resources folder.");
            return;
        }
        for (int i = 0; i < scriptableObjects.Length; i++)
        {
            scriptableObjects[i].LoadData();
        }
