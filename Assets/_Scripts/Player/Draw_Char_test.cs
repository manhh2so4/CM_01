...
public class Draw_Char_test : MonoBehaviour
{
    public int cf = 0;
    ...
    public int wpType = 0;
    int wpTypeCurrent = -1;
	 [SerializeField] public ImageID[] inforChar = new ImageID[3];
   ...
   private int[][][] CharInfo = new int[31][][]
	...
	private void Update() {
		for (int i = 0; i < 3; i++)
		{		
				CharInfo[30][i][0] = inforChar[i].ID;
				CharInfo[30][i][1] = inforChar[i].x0;
				CharInfo[30][i][2] = inforChar[i].y0;
		}
        PaintChar();
    }
